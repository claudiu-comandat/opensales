...
const JOB_NAME = 'plugin.sync_listings';
...
@Injectable()
export class SyncListingsWorker implements OnApplicationBootstrap {
  constructor(
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    await this.queue.register<SyncListingsJobData>(JOB_NAME, async (data) => {
      await this.run(data);
    });
  }

  async run(data: SyncListingsJobData): Promise<void> {
    ...
    if (!loaded.instance._actions?.syncListings) {
      this.logger.warn({ pluginId: data.pluginId }, 'Plugin does not implement syncListings');
      return;
    }
    try {
      await invokeAction(loaded.instance, 'syncListings', {});
      this.logger.log({ pluginId: data.pluginId }, 'syncListings OK');
    } catch (err) {
      this.logger.error({ err, pluginId: data.pluginId }, 'syncListings failed');
      throw err;
    }
  }

  async enqueue(pluginId: string): Promise<string | null> {
    return this.queue.enqueue<SyncListingsJobData>(JOB_NAME, { pluginId });
  }
}
